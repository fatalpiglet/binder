...
CaptureHotkeyVisual(slotIdx) {
    ...
    captureGui.SetFont("s11 c" THEME["text"], THEME["fontFamily"])
    captureGui.AddText("x30 y20 w440 c" THEME["textTitle"] " BackgroundTrans", "Нажмите нужную клавишу")
    captureGui.SetFont("s9 c" THEME["textDim"], THEME["fontFamily"])
    captureGui.AddText("x30 y50 w440 BackgroundTrans", "Примеры: F1, Ctrl+1, Alt+F5, Numpad3")
    captureGui.SetFont("s14 bold", THEME["fontMono"])
    frameCap := captureGui.AddText("x30 y90 w440 h50 Background" THEME["fieldBorder"], "")
    RoundCorners(frameCap, 440, 50, THEME["radiusSm"])
    display := captureGui.AddText("x31 y91 w438 h48 Center 0x200 Background" THEME["field"] " c" THEME["textDim"], "Ожидание…")
    RoundCorners(display, 438, 48, THEME["radiusSm"])
    ...
    captureGui.SetFont("s9", THEME["fontFamily"])
    CreateStyledButton(captureGui, 30, 180, 140, 36, "Отмена", (*) => (ih.Stop(), captureGui.Destroy()), "default").SetBackdrop(THEME["bg"])
    CreateStyledButton(captureGui, 180, 180, 140, 36, "Сброс", (*) => (display.Text := "", CapturedEditorKey := ""), "default").SetBackdrop(THEME["bg"])
    CreateStyledButton(captureGui, 330, 180, 140, 36, "Готово", (*) => (ih.Stop(), ApplyCapturedKeyToSlot(CapturedEditorKey, slotIdx, captureGui)), "primary").SetBackdrop(THEME["bg"])
...
SidebarKeyHandler(displayCtrl, statusCtrl, slotIdx, hook, vk, sc) {
    ...
    if conflict = "" {
        SetButtonVisualByControl(displayCtrl, THEME["success"])
        statusCtrl.Text := "Клавиша свободна"
        statusCtrl.Opt("c" THEME["success"])
    } else {
        SetButtonVisualByControl(displayCtrl, THEME["error"])
...
GlobalBlinkTick() {
    ...
    try {
        ...
        if blinkState {
            SetButtonVisualByControl(ctrl, THEME["warning"], THEME["field"])
            ctrl.Text := ">>> ЖДУ <<<"
        } else {
            SetButtonVisualByControl(ctrl, THEME["error"], THEME["field"])
            ctrl.Text := " НАЖМИТЕ "
        }
        ctrl.Redraw()
    } catch {
     ...
     || keyName = "LWin"    || keyName = "RWin") 
    {
        ...
        if (mods != "") {
            ...
            try {
                ...
                SetButtonVisualByControl(displayCtrl, THEME["accent"])
                displayCtrl.Redraw()
            }
        }
        return
...
FinalizeCapture(keyType, fullKey, conflictText := "") {
    ...
    try {
        ...
        SetButtonVisualByControl(ctrl, THEME["accent"], THEME["field"])
        ctrl.Redraw()
        if (conflictText != "") {
             try {
                 MainGui["Status_" keyType].Text := "Занято: " SubStr(conflictText, 1, 30)
                  SetButtonVisualByControl(ctrl, THEME["warning"])
                 ctrl.Redraw()
             }
        } else {
             try MainGui["Status_" keyType].Text := ""
...
CancelCapture(keyType) {
    ...
    try {
        ...
        SetButtonVisualByControl(ctrl, color, THEME["field"])
        ctrl.Redraw()
    }
}
...
ClearHotkey(keyType) {
    ...
    try {
        ...
        ctrl := MainGui["Display_" keyType]
        ctrl.Text := "—"
        SetButtonVisualByControl(ctrl, THEME["textMuted"])
        ctrl.Redraw()
...
ShowConflictDialog(key, conflictName, slotIdx, parentGui) {
    ...
    conflictGui.SetFont("s12 bold", THEME["fontFamily"])
    conflictGui.AddText("x30 y26 w440 c" THEME["textTitle"] " BackgroundTrans", "Конфликт клавиш")
    conflictGui.SetFont("s9 norm", THEME["fontFamily"])
    conflictGui.AddText("x30 y56 w440 c" THEME["textDim"] " BackgroundTrans", "Эта клавиша уже используется другим биндом.")

    CreateCard(conflictGui, 30, 92, 440, 92, THEME["radius"], THEME["bgElevated"])
    CreateFieldLabel(conflictGui, 50, 108, 200, "Клавиша")
    conflictGui.SetFont("s12 bold", THEME["fontMono"])
    conflictGui.AddText("x50 y124 w400 c" THEME["accent"] " BackgroundTrans", key)
    CreateFieldLabel(conflictGui, 50, 148, 200, "Занята биндом")
    conflictGui.SetFont("s10 norm", THEME["fontFamily"])
    conflictGui.AddText("x50 y162 w400 c" THEME["textBody"] " BackgroundTrans", SubStr(conflictName, 1, 45))

    conflictGui.SetFont("s10 norm", THEME["fontFamily"])
    conflictGui.AddText("x30 y204 w440 c" THEME["text"] " Center BackgroundTrans", "Заменить конфликтующую клавишу?")
    conflictGui.SetFont("s8 norm", THEME["fontFamily"])
    conflictGui.AddText("x30 y228 w440 c" THEME["textMuted"] " Center BackgroundTrans", "У старого бинда клавиша будет удалена")

    conflictGui.SetFont("s9 norm", THEME["fontFamily"])
    CreateStyledButton(conflictGui, 30, 268, 215, 38, "Заменить", (*) => ConfirmReplace(key, slotIdx, parentGui, conflictGui), "primary").SetBackdrop(THEME["bg"])
    CreateStyledButton(conflictGui, 255, 268, 215, 38, "Выбрать другую", (*) => conflictGui.Destroy(), "default").SetBackdrop(THEME["bg"])
...
ShowEditorConflictDialog(key, conflictName) {
    ...
    CreateFieldLabel(conflictGui, 50, 148, 200, "Занята биндом")
    conflictGui.SetFont("s10 norm", THEME["fontFamily"])
    conflictGui.AddText("x50 y162 w400 c" THEME["textBody"] " BackgroundTrans", SubStr(conflictName, 1, 40))
    ...
    conflictGui.SetFont("s9 norm", THEME["fontFamily"])
    CreateStyledButton(conflictGui, 30, 268, 215, 38, "Заменить", (*) => ConfirmEditorReplace(key, conflictGui), "primary").SetBackdrop(THEME["bg"])
    CreateStyledButton(conflictGui, 255, 268, 215, 38, "Выбрать другую", (*) => (conflictGui.Destroy(), EditorStartCapture()), "default").SetBackdrop(THEME["bg"])
    conflictGui.Show("w500 h330")
    conflictGui.GetPos(&gx, &gy, &gw, &gh)
    conflictGui.Move((A_ScreenWidth - gw) / 2, (A_ScreenHeight - gh) / 2)
}
